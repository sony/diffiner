...
def soundfile_info(path):
    ...
    info["samples"] = round(sfi.duration * sfi.samplerate)
...
def soundfile_loader(path, start=0, dur=None):
    ...
    if dur:
        ...
        stop = None
    audio, _ = soundfile.read(path, always_2d=True, start=start, stop=stop)
...
def torchaudio_info(path):
    ...
    info["samples"] = round(si.length / si.channels)
...
def torchaudio_loader(path, start=0, dur=None):
    ...
    if dur is None:
        ...
        sig, rate = torchaudio.load(path, num_frames=num_frames, offset=offset)
        return sig
